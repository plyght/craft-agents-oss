...
| File | Purpose |
|------|---------|
| `icon.*` | App icons (icns, ico, png, svg) — pre-macOS 26 fallback |
| `AppIcon.icon/` | macOS 26+ Liquid Glass icon source (compiled by `.github/workflows/compile-icon.yml` into Assets.car) |
| `Assets.car` | macOS 26+ compiled asset catalog. Regenerated by CI on changes to `AppIcon.icon/`. |
